...
import ru.teacherarmy.homework1.data.mapper.Mapper.toEntity
...
class SearchRepoImpl @Inject constructor(
...
): SearchResultsRepository, SafeApiRequest() {
    override suspend fun GetSearchResutls(cityName: String): List<SearchResults> {
        ...
        val response = safeApiRequest {
            searchApi.getSearchResults(searchQuery, WeatherRepoImpl.API_KEY)
        }
        return response.map {
