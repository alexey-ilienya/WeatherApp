...
internal class NetworkLayerClassSerializedNameDetector : RetrofitReturnTypeDetector() {
    ...
    class NetworkLayerDtoFieldVisitor(
    ...
    companion object {
        val ISSUE_NETWORK_LAYER_CLASS_SERIALIZED_NAME_RULE: Issue =
            Issue.create(
                id = "NetworkLayerClassSerializedNameRule",
                briefDescription = "SerializedName annotated network layer class",
                explanation =
                    "Data classes used in network layer should use `@SerializedName` annotation " +
                        "for Gson. Adding annotation prevents obfuscation errors.",
                category = CORRECTNESS,
                priority = 5,
                severity = INFORMATIONAL,
                implementation =
                    Implementation(
                        ru.teacherarmy.check.NetworkLayerClassSerializedNameDetector::class.java,
